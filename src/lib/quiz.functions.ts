...
async function addXP(supabase: any, userId: string, xp: number) {
  if (xp <= 0) return;
  const { data } = await supabase.from("user_stats").select("xp,level").eq("user_id", userId).maybeSingle();
  ...
  const { data: goal } = await supabase.from("weekly_goals").select("id,earned_xp").eq("user_id", userId).eq("week_start", week).maybeSingle();
  if (goal) {
    await supabase.from("weekly_goals").update({ earned_xp: ((goal as any).earned_xp ?? 0) + xp }).eq("id", (goal as any).id);
  } else {
    await supabase.from("weekly_goals").insert({ user_id: userId, week_start: week, earned_xp: xp });
  }
}
...
export const getLessonQuiz = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: lessonId, context }) => {
    const { data: quiz } = await context.supabase.from("quizzes").select("*").eq("lesson_id", lessonId).maybeSingle();
    if (!quiz) return null;
...
export const getLessonQuizForAdmin = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: lessonId, context }) => {
    ...
    if (!quiz) return null;
    const { data: questions } = await context.supabase
      .from("quiz_questions").select("*").eq("quiz_id", (quiz as any).id).order("sort_order");
    return { quiz, questions: questions ?? [] };
  });
...
export const upsertQuiz = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: {
    ...
    questions: Array<{ id?: string; prompt: string; choices: string[]; correct_index: number; explanation?: string; sort_order: number }>;
  }) => input)
  .handler(async ({ data, context }) => {
    const { data: existing } = await context.supabase.from("quizzes").select("id").eq("lesson_id", data.lesson_id).maybeSingle();
    let quizId = (existing as any)?.id as string | undefined;
    if (quizId) {
      await context.supabase.from("quizzes").update({
        title: data.quiz.title, passing_score: data.quiz.passing_score, xp_reward: data.quiz.xp_reward,
      }).eq("id", quizId);
    } else {
      const { data: created } = await context.supabase.from("quizzes").insert({
        lesson_id: data.lesson_id, title: data.quiz.title,
        passing_score: data.quiz.passing_score, xp_reward: data.quiz.xp_reward,
      }).select("id").single();
...
export const submitQuiz = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: quiz } = await context.supabase.from("quizzes").select("*").eq("lesson_id", data.lesson_id).maybeSingle();
    if (!quiz) throw new Error("No quiz for this lesson");
    const { data: questions } = await context.supabase
      .from("quiz_questions").select("id,correct_index,explanation").eq("quiz_id", (quiz as any).id);
    const qs = (questions ?? []) as Array<{ id: string; correct_index: number; explanation: string | null }>;
...
export const getLessonChallenge = createServerFn({ method: "GET" })
  ...
  .inputValidator((lessonId: string) => lessonId)
  .handler(async ({ data: lessonId, context }) => {
    const { data } = await context.supabase.from("challenges")
...
export const getLessonChallengeForAdmin = createServerFn({ method: "GET" })
  ...
  .inputValidator((lessonId: string) => lessonId)
  .handler(async ({ data: lessonId, context }) => {
    const { data } = await context.supabase.from("challenges").select("*").eq("lesson_id", lessonId).maybeSingle();
    return data;
  });
...
export const upsertChallenge = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: existing } = await context.supabase.from("challenges").select("id").eq("lesson_id", data.lesson_id).maybeSingle();
    const payload = {
      lesson_id: data.lesson_id, title: data.title, prompt: data.prompt,
      starter_command: data.starter_command, expected_output: data.expected_output,
      match_mode: data.match_mode, xp_reward: data.xp_reward,
    };
    if ((existing as any)?.id) {
      await context.supabase.from("challenges").update(payload).eq("id", (existing as any).id);
    } else {
...
export const submitChallenge = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: chall } = await context.supabase.from("challenges").select("*").eq("lesson_id", data.lesson_id).maybeSingle();
    ...
    else if (c.match_mode === "regex") { try { passed = new RegExp(expected).test(out); } catch { passed = false; } }
    ...
    if (passed) {
      ...
      await context.supabase.from("lesson_progress").upsert({
        user_id: context.userId, lesson_id: data.lesson_id, completed: true, score: 100, completed_at: new Date().toISOString(),
      } as any, { onConflict: "user_id,lesson_id" });
    }
    await context.supabase.from("challenge_attempts").insert({
      user_id: context.userId, challenge_id: c.id, output: out, passed, awarded_xp: awarded,
    });
...
export const getWeekly = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    const week = weekStartISO();
    let { data: goal } = await context.supabase.from("weekly_goals").select("*").eq("user_id", context.userId).eq("week_start", week).maybeSingle();
    if (!goal) {
      const { data: created } = await context.supabase.from("weekly_goals")
        .insert({ user_id: context.userId, week_start: week }).select("*").single();
      goal = created;
    }
    const { data: freezes } = await context.supabase.from("streak_freezes").select("delta").eq("user_id", context.userId);
    const tokens = (freezes ?? []).reduce((s: number, r: any) => s + (r.delta ?? 0), 0);
    const { data: notes } = await context.supabase.from("notifications").select("*").eq("user_id", context.userId).is("read_at", null).order("created_at", { ascending: false }).limit(10);
...
export const setWeeklyTarget = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    await context.supabase.from("weekly_goals").upsert({
      user_id: context.userId, week_start: week,
...
export const buyStreakFreeze = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const COST = 100;
    const { data: s } = await context.supabase.from("user_stats").select("xp").eq("user_id", context.userId).maybeSingle();
    const xp = (s as any)?.xp ?? 0;
    if (xp < COST) throw new Error(`Need ${COST} XP (have ${xp}).`);
    await context.supabase.from("user_stats").update({ xp: xp - COST }).eq("user_id", context.userId);
    await context.supabase.from("streak_freezes").insert({ user_id: context.userId, reason: "purchased", delta: 1 });
    await context.supabase.from("notifications").insert({
      user_id: context.userId, kind: "reward", title: "Streak freeze purchased 🧊",
...
export const updateProfile = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let { error } = await context.supabase.from("profiles").update(full as any).eq("id", context.userId);
    if (error) {
      ...
      const retry = await context.supabase.from("profiles").update(basic as any).eq("id", context.userId);
...
export const getLessonUnlocks = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data: courseId, context }) => {
    const { data } = await context.supabase.from("lesson_progress")
      .select("lesson_id,completed").eq("user_id", context.userId);
    const completed = new Set((data ?? []).filter((r: any) => r.completed).map((r: any) => r.lesson_id as string));
