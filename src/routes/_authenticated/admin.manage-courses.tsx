...
import { ArrowLeft, Save, Plus, Trash2, Loader2, Video, FileText, HelpCircle, CheckCircle2 } from "lucide-react";
...
function ManageCoursesPage() {
  ...
  const [questions, setQuestions] = useState<Array<{ prompt: string; choices: string[]; correct_index: number; explanation: string }>>([
  ...
  async function handleSave() {
    ...
    try {
      const courseSlug = course.slug || course.title.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      const lessonSlug = lesson.title.toLowerCase().trim().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "");
      ...
      if (validQuestions.length > 0) {
        ...
        const { data: insertedQuiz } = await supabase.from("quizzes").select("id").eq("lesson_id", lessonId).single();
        if (insertedQuiz) {
    ...
    <div className="min-h-screen bg-background">
      <header className="border-b border-border/60 sticky top-0 z-40 bg-background/80 backdrop-blur">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/admin" className="text-sm text-muted-foreground hover:text-primary flex items-center gap-1.5">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-10">
        <div>
          <h1 className="text-3xl font-bold font-display tracking-tight">Create All-in-One Course</h1>
          <p className="text-muted-foreground text-sm mt-2">
            Generate a complete course package (Course + Video + PDF + Quiz). Upload PDF and notes from your device — learners get Reading mode.
          </p>
        </div>
        ...
        <section className="space-y-4 bg-card p-6 rounded-2xl border border-border shadow-sm">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <LabeledInput label="Course Title" value={course.title} onChange={(v) => setCourse({ ...course, title: v })} placeholder="e.g. Linux Basics" />
            <LabeledInput label="URL Slug (Optional)" value={course.slug} onChange={(v) => setCourse({ ...course, slug: v })} placeholder="e.g. linux-basics" />
            <LabeledInput label="Category" value={course.category} onChange={(v) => setCourse({ ...course, category: v })} />
            <div>
          ...
          <LabeledInput label="Cover Image URL" value={course.cover_url} onChange={(v) => setCourse({ ...course, cover_url: v })} />
          <div>
        ...
        <section className="space-y-4 bg-card p-6 rounded-2xl border border-border shadow-sm">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              <LabeledInput label="Lesson Title" value={lesson.title} onChange={(v) => setLesson({ ...lesson, title: v })} />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">XP Reward</label>
              <input
                type="number"
        ...
        <section className="space-y-4 bg-card p-6 rounded-2xl border border-border shadow-sm">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            <div>
              <LabeledInput label="Quiz Title" value={quiz.title} onChange={(v) => setQuiz({ ...quiz, title: v })} />
            </div>
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Passing Score %</label>
              <input
                type="number"
            ...
            <div>
              <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Quiz XP Reward</label>
              <input
                type="number"
          ...
          <div className="mt-6 space-y-4">
            {questions.map((q, qi) => (
              <div key={qi} className="rounded-xl border border-primary/20 bg-primary/5 p-4">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-xs font-bold text-primary bg-primary/20 w-6 h-6 rounded flex items-center justify-center">Q{qi + 1}</span>
                  <input
                    value={q.prompt}
                ...
                <div className="grid grid-cols-2 gap-3 pl-8">
                  {q.choices.map((c, ci) => (
                    <label key={ci} className="flex items-center gap-2 text-sm bg-background px-3 py-1.5 rounded-lg border border-border">
                      <input
                        type="radio"
                        name={`correct-${qi}`}
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setQuestions((qs) =>
                            qs.map((x, i) =>
                              i === qi ? { ...x, choices: x.choices.map((cc, j) => (j === ci ? e.target.value : cc)) } : x,
                            ),
                          )
                        }
            ...
            <button
              onClick={() => setQuestions([...questions, { prompt: "", choices: ["", "", "", ""], correct_index: 0, explanation: "" }])}
