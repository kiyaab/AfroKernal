...
function Dashboard() {
  ...
  const { user, stats, enrolledCourses, completedLessons, examSubmissions, signOut: authSignOut } = useAuth();
      ...
      <header className="border-b border-border/60 sticky top-0 z-40 bg-background/80 backdrop-blur">
        <div className="max-w-6xl mx-auto px-6 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link to="/courses" className="text-xs font-semibold hover:text-primary">Courses</Link>
            <Link to="/exam/practice" className="text-xs font-semibold hover:text-primary">Practice Exam</Link>
            <Link to="/lab" className="text-xs font-semibold hover:text-primary">Lab</Link>
            <Link to="/chat" className="text-xs font-semibold hover:text-primary">Tutor</Link>
            <Link to="/profile" className="text-xs font-semibold hover:text-primary inline-flex items-center gap-1">
              <User className="w-3.5 h-3.5" /> Profile
            </Link>
            {isEditor && (
              <Link to="/admin" className="text-xs font-bold text-primary hover:opacity-80 inline-flex items-center gap-1">
            ...
            <ThemeToggle />
            <button onClick={handleSignOut} className="text-xs font-semibold px-3 py-1.5 rounded-lg border border-border hover:bg-secondary flex items-center gap-1.5">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground text-xs mt-1">Keep the streak alive. Master Linux command by command.</p>
          </div>
        ...
        <div className="rounded-3xl border border-border bg-card p-6 sm:p-8 space-y-4 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 pt-2">
            {CATALOG_COURSES.filter((c) => enrolledCourses.includes(c.slug) || c.slug === "linux").map((course) => {
              const doneCount = course.lessons.filter((l) => completedLessons.includes(l.id)).length;
              const pct = course.lessons.length > 0 ? Math.round((doneCount / course.lessons.length) * 100) : 0;
              const nextUnfinished = course.lessons.find((l) => !completedLessons.includes(l.id)) || course.lessons[0];

              return (
                <div key={course.id} className="rounded-2xl border border-border bg-secondary/30 p-5 space-y-3 flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-primary/10 text-primary uppercase">
                      {course.category}
                    </span>
                    <h3 className="font-bold text-sm text-foreground mt-2">{course.title}</h3>
                    <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{course.subtitle}</p>
                  </div>

                  <div className="space-y-2 pt-2 border-t border-border/60">
                    <div className="flex justify-between text-[11px] font-semibold text-muted-foreground">
                      <span>{doneCount} / {course.lessons.length} lessons</span>
        ...
        <div className="rounded-3xl border border-border bg-card p-6 sm:p-8 space-y-4 shadow-sm">
          <div className="flex items-center justify-between pb-2 border-b border-border">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-foreground">Recent Practice Exam Submissions</h2>
            </div>
            <Link to="/exam/practice" className="text-xs font-semibold text-primary hover:underline">
              Take Another Exam →
            </Link>
          </div>
          ...
          {examSubmissions.length > 0 ? (
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 pt-2">
              {examSubmissions.map((exam) => (
                <div key={exam.id} className="rounded-2xl border border-border bg-secondary/30 p-4 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="font-bold text-xs text-foreground truncate">{exam.trackLabel}</span>
                    <span className={`px-2 py-0.5 rounded-full font-bold text-[10px] ${exam.passed ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}>
                      {exam.passed ? "Passed" : "Retake"}
                    </span>
                  </div>
                  <div className="font-mono text-2xl font-black text-primary">{exam.percentage}%</div>
                  <div className="text-[11px] text-muted-foreground flex justify-between">
                    <span>{exam.score}/{exam.totalQuestions} correct</span>
            ...
            <div className="text-center py-8 text-xs text-muted-foreground space-y-3">
              ...
              <Link to="/exam/practice" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-primary text-primary-foreground font-bold">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Panel title="Weekly Learning Goal" icon={Target}>
            <div className="flex items-center justify-between text-xs font-semibold">
              <span>{goal?.earned_xp ?? currentXp} / {goal?.target_xp ?? 200} XP</span>
          ...
          <Panel title="Ask AfroKernel 24/7 Tutor" icon={Sparkles}>
            <p className="text-xs text-muted-foreground">Stuck on a command, permission syntax, or network route? Your AI mentor is grounded in the curriculum.</p>
            <Link to="/chat" className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-primary-foreground font-bold text-xs hover:brightness-110 shadow-sm">
    ...
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 rounded-2xl border border-border bg-popover shadow-xl z-50 p-2">
          {items.length === 0 && <p className="text-xs text-muted-foreground p-3">No new notifications</p>}
          {items.map((n) => (
