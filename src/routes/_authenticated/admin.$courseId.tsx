...
function CourseEditor() {
  ...
  const [newLesson, setNewLesson] = useState<{ title: string; type: LessonType }>({
    title: "",
    type: "notes",
  });
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <Link
            ...
            className="text-sm inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border hover:bg-accent"
          >
            <Eye className="w-3.5 h-3.5" /> Preview
          </Link>
          <button
            onClick={() => {
              if (confirm("Delete course and all lessons?")) del.mutate();
            }}
      ...
      <div className="grid lg:grid-cols-[minmax(0,1fr)_320px] gap-8">
        ...
        <section className="space-y-4">
          ...
          >
            ...
            {save.isPending ? "Saving…" : save.isSuccess ? "Saved" : "Save course"}
          </button>
        </section>
          ...
          <div className="space-y-1.5">
            {(lessons ?? []).map((l) => (
              <LessonRow
                key={l.id}
                lesson={l}
                onChange={() => qc.invalidateQueries({ queryKey: ["admin-lessons", courseId] })}
              />
            ))}
            {(lessons ?? []).length === 0 && (
      ...
      <section className="mt-10 space-y-6">
        {(lessons ?? []).map((l) => (
          <LessonEditor
            key={l.id}
            lesson={l}
            onSaved={() => qc.invalidateQueries({ queryKey: ["admin-lessons", courseId] })}
          />
        ))}
      </section>
    </div>
...
function LessonRow({ lesson, onChange }: { lesson: Lesson; onChange: () => void }) {
  ...
  return (
    <a
      ...
      className="flex items-center gap-2 p-2 rounded-lg border border-border bg-card hover:border-primary/60 text-sm"
    >
      ...
      <button
        onClick={(e) => {
          e.preventDefault();
          if (confirm("Delete lesson?")) del.mutate();
        }}
    ...
    >
      ...
      {form.lesson_type === "video" && (
        <div className="space-y-3 mt-3">
          <LabeledInput
            label="Video URL (YouTube embed, Vimeo, mp4)"
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        >
          ...
          {save.isPending ? "Saving…" : save.isSuccess ? "Saved" : "Save lesson"}
        </button>
      </div>
...
function QuizEditor({ lessonId }: { lessonId: string }) {
  ...
  useEffect(() => {
    ...
    setPassing(q?.passing_score ?? 70);
    setXp(q?.xp_reward ?? 25);
    setQuestions(
      (data.questions as any[]).map((qq: any) => ({
        id: qq.id,
        prompt: qq.prompt,
  ...
  return (
    <details className="mt-4 rounded-lg border border-border bg-background/40 p-4 group">
      ...
      <div className="mt-4 grid grid-cols-3 gap-3">
        ...
        <div>
          <label className="text-[11px] uppercase text-muted-foreground">Passing score %</label>
          <input
            type="number"
            value={passing}
            onChange={(e) => setPassing(parseInt(e.target.value || "0", 10))}
            className="mt-1 w-full px-3 py-2 rounded-lg bg-background border border-border text-sm"
          />
        </div>
        <div>
          <label className="text-[11px] uppercase text-muted-foreground">XP reward</label>
          <input
            type="number"
            value={xp}
      ...
      <div className="mt-4 space-y-3">
        ...
        <button
          onClick={addQuestion}
          className="text-xs px-3 py-1.5 rounded border border-dashed border-border hover:bg-accent inline-flex items-center gap-1"
        >
      ...
      <div className="mt-3 flex justify-end">
        {save.error && (
          <span className="text-xs text-destructive mr-2">{(save.error as Error).message}</span>
        )}
        <button
          onClick={() => save.mutate()}
          disabled={save.isPending}
        ...
        >
          <Save className="w-3.5 h-3.5" />{" "}
          {save.isPending ? "Saving…" : save.isSuccess ? "Saved" : "Save quiz"}
        </button>
      </div>
    </details>
  );
...
function ChallengeEditor({ lessonId }: { lessonId: string }) {
  ...
  return (
    <details className="mt-3 rounded-lg border border-border bg-background/40 p-4 group">
      <summary className="cursor-pointer text-sm font-semibold flex items-center gap-2">
        <Zap className="w-4 h-4 text-primary" /> Command challenge
        {form.expected_output ? " · configured" : ""}
      </summary>
      <div className="mt-4 space-y-3">
        <input
          value={form.title}
        ...
        <textarea
          ...
          rows={2}
          placeholder="What the learner should do…"
          className="w-full px-3 py-2 rounded-lg bg-background border border-border text-sm"
        />
        <div className="grid md:grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] uppercase text-muted-foreground">Starter command</label>
            <textarea
              value={form.starter_command}
              onChange={(e) => setForm({ ...form, starter_command: e.target.value })}
              rows={3}
              className="mt-1 w-full px-3 py-2 rounded-lg bg-background border border-border font-mono text-sm"
            />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="text-[11px] uppercase text-muted-foreground">Match mode</label>
            <select
        ...
        <div className="flex justify-end">
          ...
          >
            <Save className="w-3.5 h-3.5" />{" "}
            {save.isPending ? "Saving…" : save.isSuccess ? "Saved" : "Save challenge"}
          </button>
        </div>
      </div>
    </details>
